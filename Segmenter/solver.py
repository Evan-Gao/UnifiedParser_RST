...
class InferenceSolver(object):
    def __init__(self, model, batch_size, eval_size, epoch, lr,
    ...
    def get_batch_test(self, inputX):
        inputX = np.array(inputX)
        select_index = np.array(range(len(inputX)))
        batch_x = copy.deepcopy(inputX[select_index])
        all_lens = np.array([len(x) for x in batch_x])
        return batch_x, all_lens

    def check_accuracy(self, dataX):

        need_loop = int(np.ceil(len(dataX) / self.batch_size))
        ...
        for lp in range(need_loop):
            ...
            batch_x, all_lens = self.get_batch_test(dataX[startN:endN])

            batch_boundary, batch_boundary_start = self.model.inference(batch_x, all_lens)
            # all_ave_loss.extend([batch_ave_loss.data[0]])
            all_boundary.extend(batch_boundary)
            all_boundary_start.extend(batch_boundary_start)
        ...
        return all_boundary, all_boundary_start
    ...
    def inference(self, test_x):
        ...
        self.model.eval()
            ...
            # tr_batch_ave_loss, tr_pre, tr_rec, tr_f1, visdata1 = self.check_accuracy(self.test_train_x,
            #                                                                          self.test_train_y)

        all_boundary, all_boundary_start = self.check_accuracy(test_x)
        # _, _, all_boundary, _ = visdata
